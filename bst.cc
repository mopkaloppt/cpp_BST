...
int height( Node* x ) {
	...
	if ((x->left == NULL) && (x->right == NULL)) {
		...
		int leftP = height(x->left);
		int rightP = height(x->right);

		return std::max(leftP, rightP) + 1;
	}

	// else if (x->left != NULL) {
	// 	leftP = numNodes(x->left);
	// }
	// if (x->right != NULL) {
	// 	rightP = numNodes(x->right);
	// }
	// return (std::max(leftP,rightP) - 1);
